// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from ur_msgs:action/FollowJointTrajectoryUntil.idl
// generated code does not contain a copyright notice
#include "ur_msgs/action/detail/follow_joint_trajectory_until__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `trajectory`
#include "trajectory_msgs/msg/detail/joint_trajectory__functions.h"
// Member `path_tolerance`
// Member `goal_tolerance`
#include "control_msgs/msg/detail/joint_tolerance__functions.h"
// Member `goal_time_tolerance`
#include "builtin_interfaces/msg/detail/duration__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_Goal__init(ur_msgs__action__FollowJointTrajectoryUntil_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // trajectory
  if (!trajectory_msgs__msg__JointTrajectory__init(&msg->trajectory)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(msg);
    return false;
  }
  // path_tolerance
  if (!control_msgs__msg__JointTolerance__Sequence__init(&msg->path_tolerance, 0)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(msg);
    return false;
  }
  // goal_tolerance
  if (!control_msgs__msg__JointTolerance__Sequence__init(&msg->goal_tolerance, 0)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(msg);
    return false;
  }
  // goal_time_tolerance
  if (!builtin_interfaces__msg__Duration__init(&msg->goal_time_tolerance)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(msg);
    return false;
  }
  // until_type
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(ur_msgs__action__FollowJointTrajectoryUntil_Goal * msg)
{
  if (!msg) {
    return;
  }
  // trajectory
  trajectory_msgs__msg__JointTrajectory__fini(&msg->trajectory);
  // path_tolerance
  control_msgs__msg__JointTolerance__Sequence__fini(&msg->path_tolerance);
  // goal_tolerance
  control_msgs__msg__JointTolerance__Sequence__fini(&msg->goal_tolerance);
  // goal_time_tolerance
  builtin_interfaces__msg__Duration__fini(&msg->goal_time_tolerance);
  // until_type
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Goal__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_Goal * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // trajectory
  if (!trajectory_msgs__msg__JointTrajectory__are_equal(
      &(lhs->trajectory), &(rhs->trajectory)))
  {
    return false;
  }
  // path_tolerance
  if (!control_msgs__msg__JointTolerance__Sequence__are_equal(
      &(lhs->path_tolerance), &(rhs->path_tolerance)))
  {
    return false;
  }
  // goal_tolerance
  if (!control_msgs__msg__JointTolerance__Sequence__are_equal(
      &(lhs->goal_tolerance), &(rhs->goal_tolerance)))
  {
    return false;
  }
  // goal_time_tolerance
  if (!builtin_interfaces__msg__Duration__are_equal(
      &(lhs->goal_time_tolerance), &(rhs->goal_time_tolerance)))
  {
    return false;
  }
  // until_type
  if (lhs->until_type != rhs->until_type) {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Goal__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_Goal * input,
  ur_msgs__action__FollowJointTrajectoryUntil_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // trajectory
  if (!trajectory_msgs__msg__JointTrajectory__copy(
      &(input->trajectory), &(output->trajectory)))
  {
    return false;
  }
  // path_tolerance
  if (!control_msgs__msg__JointTolerance__Sequence__copy(
      &(input->path_tolerance), &(output->path_tolerance)))
  {
    return false;
  }
  // goal_tolerance
  if (!control_msgs__msg__JointTolerance__Sequence__copy(
      &(input->goal_tolerance), &(output->goal_tolerance)))
  {
    return false;
  }
  // goal_time_tolerance
  if (!builtin_interfaces__msg__Duration__copy(
      &(input->goal_time_tolerance), &(output->goal_time_tolerance)))
  {
    return false;
  }
  // until_type
  output->until_type = input->until_type;
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_Goal *
ur_msgs__action__FollowJointTrajectoryUntil_Goal__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Goal * msg = (ur_msgs__action__FollowJointTrajectoryUntil_Goal *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Goal));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Goal__destroy(ur_msgs__action__FollowJointTrajectoryUntil_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Goal * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_Goal *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_Goal * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `error_string`
#include "rosidl_runtime_c/string_functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_Result__init(ur_msgs__action__FollowJointTrajectoryUntil_Result * msg)
{
  if (!msg) {
    return false;
  }
  // error_code
  // until_condition_result
  // error_string
  if (!rosidl_runtime_c__String__init(&msg->error_string)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Result__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Result__fini(ur_msgs__action__FollowJointTrajectoryUntil_Result * msg)
{
  if (!msg) {
    return;
  }
  // error_code
  // until_condition_result
  // error_string
  rosidl_runtime_c__String__fini(&msg->error_string);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Result__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_Result * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // error_code
  if (lhs->error_code != rhs->error_code) {
    return false;
  }
  // until_condition_result
  if (lhs->until_condition_result != rhs->until_condition_result) {
    return false;
  }
  // error_string
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->error_string), &(rhs->error_string)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Result__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_Result * input,
  ur_msgs__action__FollowJointTrajectoryUntil_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // error_code
  output->error_code = input->error_code;
  // until_condition_result
  output->until_condition_result = input->until_condition_result;
  // error_string
  if (!rosidl_runtime_c__String__copy(
      &(input->error_string), &(output->error_string)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_Result *
ur_msgs__action__FollowJointTrajectoryUntil_Result__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Result * msg = (ur_msgs__action__FollowJointTrajectoryUntil_Result *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Result));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Result__destroy(ur_msgs__action__FollowJointTrajectoryUntil_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Result * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_Result *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_Result * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `header`
#include "std_msgs/msg/detail/header__functions.h"
// Member `joint_names`
// already included above
// #include "rosidl_runtime_c/string_functions.h"
// Member `desired`
// Member `actual`
// Member `error`
#include "trajectory_msgs/msg/detail/joint_trajectory_point__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__init(ur_msgs__action__FollowJointTrajectoryUntil_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__init(&msg->header)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(msg);
    return false;
  }
  // joint_names
  if (!rosidl_runtime_c__String__Sequence__init(&msg->joint_names, 0)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(msg);
    return false;
  }
  // desired
  if (!trajectory_msgs__msg__JointTrajectoryPoint__init(&msg->desired)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(msg);
    return false;
  }
  // actual
  if (!trajectory_msgs__msg__JointTrajectoryPoint__init(&msg->actual)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(msg);
    return false;
  }
  // error
  if (!trajectory_msgs__msg__JointTrajectoryPoint__init(&msg->error)) {
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(ur_msgs__action__FollowJointTrajectoryUntil_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // header
  std_msgs__msg__Header__fini(&msg->header);
  // joint_names
  rosidl_runtime_c__String__Sequence__fini(&msg->joint_names);
  // desired
  trajectory_msgs__msg__JointTrajectoryPoint__fini(&msg->desired);
  // actual
  trajectory_msgs__msg__JointTrajectoryPoint__fini(&msg->actual);
  // error
  trajectory_msgs__msg__JointTrajectoryPoint__fini(&msg->error);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_Feedback * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__are_equal(
      &(lhs->header), &(rhs->header)))
  {
    return false;
  }
  // joint_names
  if (!rosidl_runtime_c__String__Sequence__are_equal(
      &(lhs->joint_names), &(rhs->joint_names)))
  {
    return false;
  }
  // desired
  if (!trajectory_msgs__msg__JointTrajectoryPoint__are_equal(
      &(lhs->desired), &(rhs->desired)))
  {
    return false;
  }
  // actual
  if (!trajectory_msgs__msg__JointTrajectoryPoint__are_equal(
      &(lhs->actual), &(rhs->actual)))
  {
    return false;
  }
  // error
  if (!trajectory_msgs__msg__JointTrajectoryPoint__are_equal(
      &(lhs->error), &(rhs->error)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_Feedback * input,
  ur_msgs__action__FollowJointTrajectoryUntil_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // header
  if (!std_msgs__msg__Header__copy(
      &(input->header), &(output->header)))
  {
    return false;
  }
  // joint_names
  if (!rosidl_runtime_c__String__Sequence__copy(
      &(input->joint_names), &(output->joint_names)))
  {
    return false;
  }
  // desired
  if (!trajectory_msgs__msg__JointTrajectoryPoint__copy(
      &(input->desired), &(output->desired)))
  {
    return false;
  }
  // actual
  if (!trajectory_msgs__msg__JointTrajectoryPoint__copy(
      &(input->actual), &(output->actual)))
  {
    return false;
  }
  // error
  if (!trajectory_msgs__msg__JointTrajectoryPoint__copy(
      &(input->error), &(output->error)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_Feedback *
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Feedback * msg = (ur_msgs__action__FollowJointTrajectoryUntil_Feedback *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Feedback));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__destroy(ur_msgs__action__FollowJointTrajectoryUntil_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Feedback * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_Feedback *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_Feedback * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "ur_msgs/action/detail/follow_joint_trajectory_until__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__init(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Goal__init(&msg->goal)) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__fini(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  ur_msgs__action__FollowJointTrajectoryUntil_Goal__fini(&msg->goal);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * input,
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request *
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * msg = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__destroy(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__init(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__fini(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * input,
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response *
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * msg = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__destroy(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
#include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "ur_msgs/action/detail/follow_joint_trajectory_until__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__init(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(msg);
    return false;
  }
  // request
  if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__init(&msg->request, 0)) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(msg);
    return false;
  }
  // response
  if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__init(&msg->response, 0)) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__fini(&msg->request);
  // response
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__fini(&msg->response);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * input,
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event *
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * msg = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__destroy(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_SendGoal_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__init(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__fini(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * input,
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request *
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * msg = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__destroy(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "ur_msgs/action/detail/follow_joint_trajectory_until__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__init(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Result__init(&msg->result)) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__fini(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  ur_msgs__action__FollowJointTrajectoryUntil_Result__fini(&msg->result);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * input,
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response *
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * msg = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__destroy(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
// already included above
// #include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "ur_msgs/action/detail/follow_joint_trajectory_until__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__init(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(msg);
    return false;
  }
  // request
  if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__init(&msg->request, 0)) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(msg);
    return false;
  }
  // response
  if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__init(&msg->response, 0)) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__fini(&msg->request);
  // response
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__fini(&msg->response);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * input,
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event *
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * msg = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__destroy(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_GetResult_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "ur_msgs/action/detail/follow_joint_trajectory_until__functions.h"

bool
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__init(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Feedback__init(&msg->feedback)) {
    ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__fini(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  ur_msgs__action__FollowJointTrajectoryUntil_Feedback__fini(&msg->feedback);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * input,
  ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!ur_msgs__action__FollowJointTrajectoryUntil_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage *
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * msg = (ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage));
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__destroy(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__init(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * data = NULL;

  if (size) {
    data = (ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage *)allocator.zero_allocate(size, sizeof(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__fini(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence *
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * array = (ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence *)allocator.allocate(sizeof(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__destroy(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__are_equal(const ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * lhs, const ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence__copy(
  const ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * input,
  ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage * data =
      (ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!ur_msgs__action__FollowJointTrajectoryUntil_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
